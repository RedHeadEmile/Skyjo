...
export class ApiService {
     ...
     * @return No Content
     */
    storeGameAction(body: GameActionViewModel): Observable<void> {
...
export class SkyjoRoomViewModel implements ISkyjoRoomViewModel {
    ...
    init(_data?: any) {
        if (_data) {
            ...
            if (Array.isArray(_data["pristineCards"])) {
                this.pristineCards = [] as any;
                for (let item of _data["pristineCards"])
                    this.pristineCards!.push(item);
            }
            if (Array.isArray(_data["discardedCards"])) {
                this.discardedCards = [] as any;
                for (let item of _data["discardedCards"])
                    this.discardedCards!.push(item);
            }
    ...
    toJSON(data?: any) {
        ...
        if (Array.isArray(this.pristineCards)) {
            data["pristineCards"] = [];
            for (let item of this.pristineCards)
                data["pristineCards"].push(item);
        }
        if (Array.isArray(this.discardedCards)) {
            data["discardedCards"] = [];
            for (let item of this.discardedCards)
                data["discardedCards"].push(item);
        }
...
export class GameActionViewModel implements IGameActionViewModel {
    type!: GameActionViewModelType;
    cardIndex?: number;

    [key: string]: any;

    constructor(data?: IGameActionViewModel) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): GameActionViewModel {
        data = typeof data === 'object' ? data : {};
        let result = new GameActionViewModel();
        result.init(data);
        return result;
    }
...
export interface IGameActionViewModel {
    type: GameActionViewModelType;
    cardIndex?: number;
...
export enum GameActionViewModelType {
    DRAW_A_CARD = "DRAW_A_CARD",
    FLIP_A_CARD = "FLIP_A_CARD",
    EXCHANGE_A_CARD = "EXCHANGE_A_CARD",
}
